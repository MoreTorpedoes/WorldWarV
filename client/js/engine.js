    ...
    else if (wwv.game_state.nukes)
    {
        ...
        for (var i=0; i<NK.length; i++)
        {
            ...
            if (t >= 1.0)
            {
                ...
                wwv.particle.nuke_explosion( NK[i] );
            ...
            wwv.particle.fire_particle( { x: NK[i].x - Math.cos(a2) * NK[i].z * 35, y: NK[i].sy - Math.sin(a2) * NK[i].z * 35, z: NK[i].z * 60.0 }, { x: Math.cos(a2), y: Math.sin(a2), z: 0.0 } );//, null, tint, alpha, t, sz );
            a2 += Math.random() * Math.PI / 2 - Math.PI / 4;
            wwv.particle.fire_particle( { x: NK[i].x - Math.cos(a2) * NK[i].z * 20, y: NK[i].sy - Math.sin(a2) * NK[i].z * 20, z: NK[i].z * 60.0 }, { x: Math.cos(a2), y: Math.sin(a2), z: 0.0 }, null, 0xff0000, null, 0.75);
            a2 += Math.random() * Math.PI / 2 - Math.PI / 4;
    ...
    for (var i=0; i<map.C.length; i++)
    {
        for (var j=0; j<map.C[i].CIT.length; j++)
        {
            ...
            if (Math.pow(Math.random(), 2.0) < dmg)
            {
                ...
                var r = Math.random();
                wwv.particle.fire_particle( { x: map.C[i].CIT[j].x + Math.cos(a2) * r * 25, y: map.C[i].CIT[j].y + Math.sin(a2) * r * 25 + 20, z: 30 }, { x: Math.cos(a2), y: Math.sin(a2), z: 5.0 }, null, map.C[i].CIT[j].dead ? 0x808080 : 0xc0c0c0 );//, null, tint, alpha, t, sz );
            }
        }
    }

    wwv.prtImage = wwv.particle.render_particles(wwv.prtImage);
